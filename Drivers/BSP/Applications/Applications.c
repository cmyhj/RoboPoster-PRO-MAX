...
uint8_t USART_Receive(void)
{
    ...
    for(int i=0;i<9;++i)
    {
        if(g_usart_rx_buf[(i+5)%9] == 'c' && g_usart_rx_buf[(i+6)%9] == 'k' && g_usart_rx_buf[(i+7)%9] =='y' && g_usart_rx_buf[(i+8)%9] == 'f')
        {
           uart_receive[0] = g_usart_rx_buf[(i)%9];
           uart_receive[1] = g_usart_rx_buf[(i+1)%9];
           uart_receive[2] = g_usart_rx_buf[(i+2)%9];
           uart_receive[3] = g_usart_rx_buf[(i+3)%9];
           uart_receive[4] = g_usart_rx_buf[(i+4)%9];
           return 1;
        }
    }
    return 0;
...
{
    lcd_show_string(block->position.x,block->position.y,block->size,640,860,block->str,block->color);
}
...
void Send_Chess_info(int press)
{
    for(int i=0;i<500;i++)
        if(press != 0)
        {
