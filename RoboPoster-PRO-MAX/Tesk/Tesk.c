...
void Car_get()
{
	...
	for (int8_t i = 0; i < 3; i++)
	{
	  ...
	  while(Get_cc[3]==0 && Get_cc[2]==0);
		while(fabsf(10.0f * Get_cc[2] / Get_cc[5])>5 || fabsf(10.0f * Get_cc[3] / Get_cc[5])>5){//物料不在中心或物料非任务目标
			osDelay(1);
		...
		while(fabsf(10.0f * Get_cc[2] / Get_cc[5])>5 || fabsf(10.0f * Get_cc[3] / Get_cc[5])>5 ||Get_cc[4]!=Test[i]){
			osDelay(1);
		}
		Sink_firstplat;
		Claw;
